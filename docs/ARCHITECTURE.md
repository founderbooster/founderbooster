...
- ports.json
- cloudflared.pid
- state.json (stable interface for plugins/tools, written after `fb bootstrap`)

App repos remain unchanged.
All state is machine-local and can be regenerated by re-running `fb bootstrap`.
`state.json` includes canonical paths for these artifacts; `tunnel.token` remains separate and sensitive.
